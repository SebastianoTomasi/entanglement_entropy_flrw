# -*- coding: utf-8 -*-
"""
Created on Tue Nov 26 17:49:13 2024
...
class CosmologicalModel:
    def __init__(self):
        """Initialize the cosmological model by performing the necessary calculations."""
        # Initialize instance attributes
        self.scale_factor_t = None
        self.hubble_function_t = None
        self.comoving_horizon_t = None
        self.cut_off_t = None
    ...
    def compute_cosmology(self):
        """Compute the cosmological functions based on the parameters."""
        # We distinguish flat and non-flat cases. Even if the formalism is the same,
        ...
        if par.cosmology in ["eds", "lcdm", "rad"]:
            ...
            dark_density_evolution_a = sp.interpolate.interp1d(
            ...
            if __name__ == "__main__" or par.debug_level >= 2:  # For debugging
                pl.plot(
                    [scale_parameter_values, friedmann_equation_integrand(scale_parameter_values)],
                    func_to_compare=lambda y: np.e ** (3 * y / 2),
                    title="Fr. Eq. integrand"
                )

            hubble_constant_times_t = nm.integrate(
                f=friedmann_equation_integrand,
            ...
            universe_age = time[nm.find_closest_index(scale_parameter_values, 1.)]
            if __name__ == "__main__":
                print("Age of the universe = ", universe_age)
            ...
            scale_factor_t_temp = sp.interpolate.interp1d(
                time, scale_parameter_values,
                fill_value="extrapolate", assume_sorted=True, kind="quadratic"
            )
            cosmic_time_a = sp.interpolate.interp1d(
                scale_parameter_values, time,
                fill_value="extrapolate", assume_sorted=True, kind="quadratic"
            )

            h = nm.rms_neigbour_distance(time)  # Compute average spacing between time points
            new_length = int(2 / h)  # Use h to obtain the new time array length
            time = np.linspace(time[0], time[-1], new_length)  # Build a new time array with equally spaced points
            scale_parameter_values = scale_factor_t_temp(time)  # Corresponding scale parameter values

            scale_parameter_derivative_numerical_t = nm.derivate(
                scale_factor_t_temp, time[0], time[-1], points=new_length, n=1, order=2
            )

            skip = 2  # Skip the first two values because those are incorrect
            ...
            time = time[skip:]  # Adjust time array to match dimensions
            scale_parameter_values = scale_parameter_values[skip:]
            hubble_function_numerical_t = np.array([
                time,
                scale_parameter_derivative_numerical_t[1] / scale_parameter_values
            ])
            hubble_function_t_temp = sp.interpolate.interp1d(
                hubble_function_numerical_t[0], hubble_function_numerical_t[1],
                fill_value="extrapolate", assume_sorted=True, kind="quadratic"
            )
            if __name__ == "__main__" or par.debug_level >= 2:  # For debugging
                pl.plot(
                    [hubble_function_numerical_t[0], hubble_function_numerical_t[1]],
                    title="Hubble function",
                    xlabel=["t "], ylabel=["H(t)"],
                    yscale="log", xscale="log",
                    legend=["Numerical", "EdS"],
            ...
            """Compute the horizon according to the user choice"""
            if par.horizon_type == "fixed_comoving":
                def aux(t):
                    return par.H0_c
                comoving_horizon_t_temp = np.vectorize(aux)
            elif par.horizon_type == "fixed_physical":
                def comoving_horizon_t_temp(t):
                    a = scale_factor_t_temp(t)
                    return par.H0_c / a
                ...
                comoving_horizon_numerical_a = nm.integrate(
                    horizon_integrand_a,
                    par.horizon_a_min,
                    par.horizon_a_max,
                ...
                comoving_horizon_a = sp.interpolate.interp1d(
                    comoving_horizon_numerical_a[0],
                    par.H0_c * comoving_horizon_numerical_a[1],
                    fill_value="extrapolate", assume_sorted=True
                )

                comoving_horizon_t_temp = sp.interpolate.interp1d(
                    cosmic_time_a(comoving_horizon_numerical_a[0]),
                    par.H0_c * comoving_horizon_numerical_a[1],
                    fill_value="extrapolate", assume_sorted=True
                )
            else:
                raise Exception(f"horizon_type={par.horizon_type} not recognized.")

            """Define the cut-off function, depends on user choice."""
            if par.cut_off_type == "fixed_comoving":
                def aux(t):
                    return par.cut_off
                cut_off_t_temp = np.vectorize(aux)
            elif par.cut_off_type == "fixed_physical":
                def cut_off_t_temp(t):
                    a = scale_factor_t_temp(t)
            ...
            if par.horizon_type == "particle_horizon":
                ...
                pl.plot(
                    [a_for_plot, comoving_horizon_t_temp(a_for_plot)],
                    dotted=True,
            ...
            # Assign computed functions to class attributes
            self.scale_factor_t = scale_factor_t_temp
            self.hubble_function_t = hubble_function_t_temp
            self.comoving_horizon_t = comoving_horizon_t_temp
            self.cut_off_t = cut_off_t_temp
            ...
            self.scale_factor_t = np.vectorize(scale_factor_tt)

            def hubble_function_tt(t):
                return par.hubble_constant
            self.hubble_function_t = np.vectorize(hubble_function_tt)

            def H0_c_func(t):
                return par.H0_c
            self.comoving_horizon_t = np.vectorize(H0_c_func)

            def const_cut_off(a):
                return par.cut_off
            self.cut_off_t = np.vectorize(const_cut_off)
           ...
           """Compute the hubble function H(t)=\dot{a}/a and the second derivative of a"""
           scale_factor_t_temp=sp.interpolate.interp1d(unique_time, scale_means,
                                             fill_value="extrapolate", assume_sorted=True,kind="quadratic")
           ...
           scale_parameter_values=scale_factor_t_temp(time)#Compute the corresponding scale parameter values
           
           # print(time[0],time[-1])
           scale_parameter_derivative_numerical_t=nm.derivate(scale_factor_t_temp,time[0],time[-1],points= new_lenght,n=1,order=2)
           ...
           hubble_function_t_temp=sp.interpolate.interp1d(hubble_function_numerical_t[0], hubble_function_numerical_t[1],
                                             fill_value="extrapolate", assume_sorted=True,kind="quadratic")
           if __name__ == "__main__" or par.debug_level>=2:#For debug
           ...
           if par.horizon_type=="fixed_comoving":
               ...
               def comoving_horizon_t(t):
                   a=scale_factor_t_temp(t)
                   return par.H0_c/a
               self.comoving_horizon_t=comoving_horizon_t
               ...
               self.comoving_horizon_t=sp.interpolate.interp1d(cosmic_time_a(comoving_horizon_numerical_a[0]),par.H0_c*comoving_horizon_numerical_a[1],
                                                   fill_value="extrapolate", assume_sorted=True)
           else:
           ...
           if par.horizon_type=="particle_horizon":
            ...
            #%% FLAT CASE
            """Define trivial functions in the flat spacetime case."""
            def one(t):
                return 1
            self.scale_factor_t = np.vectorize(one)

            def zero(t):
                return 0
            self.hubble_function_t = np.vectorize(zero)
...
if __name__=="__main__":
    ...
    cosmo_model = CosmologicalModel()
    
    # Access the computed functions
    scale_factor_t = cosmo_model.scale_factor_t