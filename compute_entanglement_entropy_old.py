...
Todos:
    ...
    -Vectorization of the ode solver, to make it solve for all gamma_l2 values at 
    once instead that for each gamma_lj2.
"""
...
def run():
    ...
    for i in range(1,len(par.times)):#Starts from one because of the definition of times.
        ...
        for l in range(0,par.l_max+1):#Cycle on the angular momentum index
            ...
            # if l%100==0:
            #     print(np.round(np.dot(np.dot(U,coupling_matrix_l),UT),4))
            ...
            for j,gamma_lj2 in enumerate(gamma_l2):# j gores from 0 to par.N-1 
                ...
                def omega_lj2(t):
                    """Takes in input the cosmological time t and returns omega_lj2(t), (\omega_{lj}^2 in the paper),
                    ...
                    b=cosm.cut_off_t(t)#Allow for time dependend cut off: ex. if fixed physical
                    mu=par.mu
                    c=par.c
                    muab2=(mu*a*b)**2
                    return (c/a)**2*(gamma_lj2+muab2)
                
                """In the first iteration we have to impose the flat spacetime harmonic oscillator initial conditions"""
                if i==1:
                ...
                We solve it between the previous time and the actual time. As initial conditions
                we use the result saved in the previous time ."""
                ...
                if np.sign(rho_lj_t)!=np.sign(rho_t_prec[l][j]):
                    ...
                    but the rk4 method fails to keep the sign the same. We can adjust this with those operation.
                    If the tolerances in the rk4 solver are tight enough, the alghorithm should
                    never come in here, but if it does, this will significantly improve results."""
                    rho_lj_t*=-1
                    drho_lj_t*=-1
                    if par.verbose>=0:
                ...
                harmonic oscillator initial condtions. We keep the option of not performing this calculation
                ...
                """Save the values to reuse tham in the next time i+1."""
                rho_t_prec[l][j],drho_t_prec[l][j]=rho_lj_t,drho_lj_t
                
                """We construct the ground state matrix computed at t: diagonal_sigma_l_t= the diagonal of \boldsymbol{\Sigma}^l(t) in the paper."""
                diagonal_sigma_l_t.append(1/par.hbar*( 1/rho_lj_t**2-1j*M_t(t)*drho_lj_t/rho_lj_t ) )
            
            
            """After the cycle is compleated, we can construct the actual ground state matrix sigma_l_t=\boldsymbol{\Sigma}^l(t) and
            ...
            for k,n in enumerate(par.n_values):
                if n==par.n_min or n==par.n_max:
                    ...
                    A_real_inverse_times_B=np.dot(A_real_inverse, B)
                    ...
                    if par.verbose>=3 and np.any(np.imag(eigenvalues) != 0):  # Check if there are non-zero imaginary parts
